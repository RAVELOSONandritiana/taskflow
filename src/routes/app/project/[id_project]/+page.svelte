<script lang="ts">
	...
	import SwotDiagram from '$lib/components/user/app/SwotDiagram.svelte';
	import GanttChart from '$lib/components/user/app/GanttChart.svelte';
	import RiskMatrix from '$lib/components/user/app/RiskMatrix.svelte';
	...
	let project = $derived(
		$projects.find((p) => p.id === id_project) || {
			id: id_project,
	...
	let activeTab = $state('tasks'); // 'overview', 'tasks', 'diagram', 'swot', 'gantt', 'risk'
	...
	function saveSettings() {
		projects.update((all) =>
			all.map((p) => (p.id === id_project ? { ...p, title: editTitle, description: editDesc } : p))
		);
		showSettings = false;
	}
...
<div class="bg-gray-50/30 dark:bg-transparent">
	...
	<section class="px-8 pt-8">
		<div class="mb-6 flex items-start justify-between">
			<div>
				<nav
					class="mb-2 flex items-center gap-2 text-[10px] font-black tracking-widest text-gray-400 uppercase"
				>
					<a href="/app/project" class="transition-colors hover:text-indigo-500">Projects</a>
		...
		{:else}
			<div class="grid grid-cols-1 gap-8 lg:grid-cols-3" in:fly={{ y: 20, duration: 400 }}>
				...
				<div class="space-y-8 lg:col-span-2">
					...
					<section
						class="flex flex-col gap-6 rounded-[2rem] border border-gray-100 bg-white p-8 shadow-sm md:flex-row md:items-center dark:border-gray-800 dark:bg-gray-900/50"
					>
						...
						<div class="space-y-2">
							<div class="flex items-center gap-2">
								...
								<h3 class="text-sm font-black tracking-widest text-gray-400 uppercase">
									Project Health: On Track
								</h3>
							</div>
							<h4 class="text-xl font-black text-gray-900 dark:text-white">
								Mid-journey through {project?.title}
							</h4>
							<p class="text-sm leading-relaxed font-medium text-gray-500 dark:text-gray-400">
								You've completed half of your planned tasks. The team velocity is stable, and no
								clinical blockers are currently identified. Keep the momentum!
							</p>
						</div>
					</section>
					...
					<div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
						{#each stats as stat}
							<div
								class="flex items-center gap-4 rounded-3xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900/50"
							>
								<div
									class="flex h-12 w-12 items-center justify-center rounded-2xl {stat.bg} {stat.color}"
								>
									{#if stat.icon === 'check'}
										<svg
								...
								<div>
									...
									<p class="text-xl font-black text-gray-900 dark:text-white">
										{stat.value}<span class="text-xs text-gray-300"> / {stat.total}</span>
									</p>
								</div>
							</div>
						{/each}
					...
					<!-- Recent Activity -->
					<section
						class="rounded-[2rem] border border-gray-100 bg-white p-8 shadow-sm dark:border-gray-800 dark:bg-gray-900/50"
					>
						<div class="mb-6 flex items-center justify-between">
							...
							<button
								class="text-[10px] font-black tracking-widest text-indigo-500 uppercase transition-colors hover:text-indigo-600"
								>View All</button
							>
						</div>
							...
							{#each activities as activity}
								<div class="flex items-start gap-4">
									<img
										src={Avatar}
										alt={activity.user}
										class="h-8 w-8 rounded-xl border border-gray-100 object-cover dark:border-gray-800"
									/>
									<div class="flex flex-col">
										<p class="text-sm font-medium text-gray-600 dark:text-gray-400">
											<span class="font-black text-gray-900 dark:text-white">{activity.user}</span>
											{activity.action}
											<span class="font-bold text-indigo-500">{activity.target}</span>
										</p>
									...
									<div
										class="ml-auto flex h-7 w-7 items-center justify-center rounded-lg bg-gray-50 text-gray-400 dark:bg-gray-800"
									>
				...
				<div class="space-y-8">
					...
					>
						<div class="mb-6 flex items-center justify-between">
							...
							<button
								class="flex h-8 w-8 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30"
						...
						<div class="space-y-5">
							{#each members as member}
								<div class="group flex items-center justify-between">
									<div class="flex items-center gap-3">
										<div class="relative">
											<img
												src={Avatar}
												alt={member.name}
												class="h-10 w-10 rounded-2xl border border-gray-100 object-cover shadow-sm dark:border-gray-800"
											/>
											<div
												class="absolute -right-0.5 -bottom-0.5 h-3 w-3 rounded-full border-2 border-white {member.status ===
												'online'
													? 'bg-green-500'
													: member.status === 'away'
														? 'bg-orange-400'
														: 'bg-gray-300'} dark:border-gray-900"
											></div>
										</div>
										<div class="flex flex-col">
									...
									<button
										class="text-gray-400 opacity-0 transition-all group-hover:opacity-100 hover:text-indigo-500"
						...
						<button
							class="mt-8 w-full rounded-2xl bg-gray-50 py-3 text-xs font-black tracking-widest text-gray-500 uppercase transition-all hover:bg-gray-100 active:scale-[0.98] dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"
						>
							Manage Access
						</button>
					</section>
					...
					<!-- Quick Summary / Links -->
					<section
						class="rounded-[2rem] bg-indigo-600 p-8 text-white shadow-2xl shadow-indigo-200 dark:shadow-none"
					>
						<h3 class="mb-4 text-sm font-black tracking-[0.2em] uppercase opacity-60">Pro Tip</h3>
						<p class="text-sm leading-relaxed font-bold">
							Connect with your team in the <a
								href="/app/chat"
								class="underline decoration-indigo-300 underline-offset-4 hover:text-indigo-200"
								>real-time chat</a
							> or use our AI assistant to optimize your workflow strategy.
						</p>
							...
							<div class="flex -space-x-2">
								{#each [1, 2, 3] as i}
									<img
										src={Avatar}
										alt="Member"
										class="h-8 w-8 rounded-full border-2 border-indigo-600 object-cover"
									/>
								{/each}
							</div>
...
<Dialog bind:open={showSettings} onClose={() => (showSettings = false)}>
	...
	>
		<div class="space-y-1">
			...
			<p class="text-xs font-bold tracking-widest text-indigo-500 uppercase">
...
<Dialog bind:open={showShare} onClose={() => (showShare = false)}>
	...
	>
		...
		<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-900">
			...
			<div
				class="flex items-center gap-2 overflow-hidden rounded-xl bg-white p-1 ring-1 ring-gray-100 dark:bg-gray-800 dark:ring-gray-700"
			>
				<input
					type="text"
					readonly
		...
		<div class="flex flex-col gap-2">
			<div
				class="flex items-center gap-3 rounded-2xl border border-gray-100 p-4 dark:border-gray-800"
			>
				<div
					class="flex h-10 w-10 items-center justify-center rounded-xl bg-green-50 text-green-600 dark:bg-green-900/20"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						class="h-5 w-5"
						...
						><path
							...
							stroke-linejoin="round"
							stroke-width="2"
							d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-7.618 3.04M12 21a9.003 9.003 0 008.367-5.618m0 0L12 18l-8.367-2.618m0 0A9.003 9.003 0 0012 21"
						/></svg
					>
				</div>
				<div class="flex flex-col">
					...
					<span class="text-[9px] font-bold tracking-widest text-gray-400 uppercase"
						>Off (Invite only)</span
					>
				</div>
				<button
					class="ml-auto h-5 w-9 rounded-full bg-gray-200 p-1 transition-colors dark:bg-gray-800"
