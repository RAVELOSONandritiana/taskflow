<script lang="ts">
	...
	const providers: Provider[] = [
		{
			id: 'local',
			name: 'Private Local (LM Studio)',
			url: 'http://localhost:1234/v1/chat/completions',
			description: 'Runs entirely on your machine. Maximum privacy, requires LM Studio.'
		},
		{
			id: 'puter',
			name: 'Cloud AI (Puter.js)',
			description: 'Fast, free, and hosted. No setup required.'
		}
	];
...
<div class="flex h-full flex-col overflow-hidden bg-white dark:bg-gray-950">
	...
	>
		<div class="flex items-center gap-4">
			<div
				class="flex h-10 w-10 items-center justify-center rounded-2xl bg-indigo-500 text-white shadow-lg shadow-indigo-200 dark:shadow-none"
			>
				<svg
			...
			<div class="flex items-center gap-1 rounded-2xl bg-gray-50 p-1 dark:bg-gray-900">
				{#each providers as provider}
					<button
						...
						class="rounded-xl px-4 py-2 text-[10px] font-black tracking-wider uppercase transition-all
	...
	<!-- Chat History -->
	<div class="custom-scrollbar flex-1 space-y-6 overflow-y-auto p-6" bind:this={chatContainer}>
		...
		{#if isTyping}
			<div class="flex items-start gap-4" in:fade>
				<div
					class="flex h-9 w-9 animate-pulse items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						class="h-5 w-5"
				...
				<div
					class="flex items-center gap-2 rounded-2xl bg-gray-50 px-5 py-3 text-sm font-medium text-gray-500 dark:bg-gray-900"
				>
					Processing request...
					<div class="flex gap-1">
						<div class="h-1 w-1 animate-bounce rounded-full bg-gray-400"></div>
						<div
							class="h-1 w-1 animate-bounce rounded-full bg-gray-400 [animation-delay:0.2s]"
						></div>
						<div
							class="h-1 w-1 animate-bounce rounded-full bg-gray-400 [animation-delay:0.4s]"
						></div>
					</div>
				</div>
			</div>
	...
	<footer class="border-t border-gray-100 p-6 dark:border-gray-800">
		<form onsubmit={handleSubmit} class="mx-auto flex max-w-4xl items-end gap-4">
			<div class="group relative flex-1">
				<textarea
					bind:value={newMessage}
					placeholder={selectedProviderId === 'local'
						? 'Message local AI (LM Studio)...'
						: 'Ask Puter Cloud AI anything...'}
					rows="1"
					onkeydown={(e) => {
					...
					class="max-h-[200px] w-full resize-none overflow-hidden rounded-2xl border-none bg-gray-50 px-6 py-4 text-sm font-medium ring-1 ring-indigo-500/30 transition-all outline-none group-focus-within:ring-2 group-focus-within:ring-indigo-500 dark:bg-gray-900 dark:ring-indigo-500/20"
				></textarea>
				<div class="absolute top-1/2 right-4 flex -translate-y-1/2 items-center gap-2">
					<span
						class="hidden text-[10px] font-bold tracking-widest text-gray-400 uppercase md:block"
						>Enter to Send</span
					>
				</div>
			</div>
			<button
				...
				class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-indigo-600 text-white shadow-xl shadow-indigo-100 transition-all hover:bg-indigo-700 hover:shadow-indigo-200 active:scale-90 disabled:scale-95 disabled:opacity-50 dark:shadow-none"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					class="h-6 w-6"
...
<Dialog bind:open={showHelp} onClose={() => (showHelp = false)}>
	...
	>
		<div class="space-y-3 text-center">
			<div
				class="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-indigo-50 text-indigo-500 dark:bg-indigo-900/20"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					class="h-10 w-10"
		...
		<div class="grid grid-cols-1 gap-4">
			<div
				class="rounded-3xl border border-gray-100 bg-gray-50/50 p-6 dark:border-gray-800 dark:bg-gray-900/50"
			>
				<h3
					class="mb-3 flex items-center gap-2 text-xs font-black tracking-widest text-indigo-500 uppercase"
				>
					<div class="h-2 w-2 rounded-full bg-indigo-500"></div>
					Option 1: Puter Cloud
				</h3>
				<p class="text-sm leading-relaxed text-gray-600 dark:text-gray-400">
					Works instantly! No configuration or API keys needed. Perfect for quick questions and
					online tasks.
				</p>
			</div>
			...
			>
				<h3
					class="mb-3 flex items-center gap-2 text-xs font-black tracking-widest text-orange-500 uppercase"
				>
					<div class="h-2 w-2 rounded-full bg-orange-500"></div>
					Option 2: LM Studio (Local)
				</h3>
				<p class="text-sm leading-relaxed text-gray-600 dark:text-gray-400">
					Start the <b>Local Server</b> inside LM Studio on port <b>1234</b>. Your data never leaves
					your machine.
				</p>
			</div>
		</div>
...
<Dialog bind:open={showSettings} onClose={() => (showSettings = false)}>
	...
	>
		<div class="space-y-1">
			...
			<p class="text-xs font-bold tracking-widest text-indigo-500 uppercase">Provider Details</p>
		</div>
		...
		<div class="rounded-2xl bg-gray-50 p-4 dark:bg-gray-900">
			<div
				class="flex items-center justify-between text-[10px] font-black tracking-widest text-gray-400 uppercase"
			>
				<span>Status</span>
