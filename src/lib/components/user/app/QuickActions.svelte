<script lang="ts">
	...
	const actions = [
		...
		{
			title: 'Upload Music',
			description: 'Add new tracks to player',
		...
		{
			title: 'View Settings',
			description: 'Personalize TaskFlow',
...
<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
	{#each actions as action}
		<button
			...
			class="group flex flex-col items-start rounded-xl border border-gray-100 bg-white p-5 text-left transition-all hover:border-indigo-100 hover:shadow-lg dark:border-gray-800 dark:bg-gray-800 dark:hover:border-indigo-900"
		>
			<div class="mb-4 rounded-lg {action.color} p-2.5 text-white transition-transform group-hover:scale-110">
				<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					{@html action.icon}
				</svg>
			</div>
			<h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">{action.title}</h3>
