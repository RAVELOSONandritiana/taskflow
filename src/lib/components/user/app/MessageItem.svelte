		...
		{#if !isMe}
			<div class="relative shrink-0">
				<img
					src={Avatar}
					alt={sender}
					class="h-9 w-9 rounded-2xl border-2 border-white bg-indigo-50 object-cover shadow-sm dark:border-gray-800 dark:bg-gray-900"
				/>
				<div
					class="absolute -right-0.5 -bottom-0.5 h-3 w-3 rounded-full border-2 border-white bg-green-500 dark:border-gray-950"
				></div>
			</div>
		{/if}
		...
		<div class="flex flex-col {isMe ? 'items-end' : 'items-start'}">
			{#if !isMe}
				<div class="mb-1.5 flex items-center gap-2 px-1">
					<span
						class="text-[11px] font-black tracking-widest text-gray-900 uppercase dark:text-gray-400"
						>{sender}</span
					>
			...
			<div
				...
				{isMe
					? 'rounded-br-none bg-indigo-600 text-white shadow-indigo-100 dark:shadow-none'
					: 'rounded-bl-none border border-gray-100 bg-white text-gray-800 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-100'}"
			>
				<div
					class="prose prose-sm max-w-none {isMe
						? 'text-white prose-invert'
						: 'dark:prose-invert'} prose-p:leading-relaxed prose-pre:bg-transparent prose-pre:p-0"
				>
					{@html html}
				</div>
				...
				{#if isMe}
					<div class="mt-2 flex justify-end">
						<span class="text-[9px] font-black tracking-widest uppercase opacity-50">{time}</span>
					</div>
				{/if}
			</div>
		</div>
